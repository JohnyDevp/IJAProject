...
public class ClassDiagramController {
     ...
     * undone-able operations are: renaming object, adding attribute, adding operation
     */
    ...
    public void btnSave(){
        ...
        if (getLoadedFilePath().equals("")){
            try{
                ...
                } else {return; } //return if no file chosen
    ...
    @FXML
    public void btnUndo(){
        if (this.listOfoperationsToBeUndone.size() == 0) Errors.showAlertDialog("Nothing to be undone!", Alert.AlertType.WARNING); //nothing to be undone
        else {
            ...
            if (!done) {
                Errors.showAlertDialog("Operation " + undo.getOperationType() + "cannot be undone.", Alert.AlertType.WARNING);
            } else {
    ...
    /**
     * method for notifying all other sequence diagrams about all changes that has been maid
     */
    public void notifySequenceDiagramsAboutChanges(){
        //loop through list of all sequence diagram controllers and tell them to update themselves
